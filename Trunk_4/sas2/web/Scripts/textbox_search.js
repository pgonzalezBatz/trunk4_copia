...
function textboxSearch(url, searchBox, oneElementToAppend, manyElementsToAppend, elementContainer) {
    var fDisplay = function (sB) {
        ...
        if (typeof (elementContainer) == 'undefined') {
            elementContainer = '<div id="divavoidcolision" style="position:absolute;left:' + pos.left.toString() + 'px;top:' + (pos.top + sB.outerHeight()).toString() + 'px;display:block;width:48%;height:200px;background-color:Gray;overflow:auto;background-color:#EEE;border:1px solid #8FABFF;z-index:10;"></div>';
        };
