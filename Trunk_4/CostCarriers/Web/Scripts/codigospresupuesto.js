function initBusquedaCodigosPresupuesto(text, hidden, div, url) {
    $(function () {        
        var texto = $('#' + text);
        var hid = $('#' + hidden);
        var divAux = $('#' + div);
        ...
        var bindBehaviors = function (val) {
            $('#' + val.Codigo + texto.attr('id')).click(function () {
                texto.val(val.DescripcionCompleta);
                hid.attr("value", val.Codigo);
                    ...
                    $.getJSON(url + "?q=" + encodeURIComponent(text), function (json) {
                        ...
                        if (json.length == 0) {
                            ...
                            $.each(json, function (key, val) {
                                ...
                                {
                                    addUsuario(val.Id, val.DescripcionCompleta);
                                    if (texto && texto.hasClass('auto-no-seleccionado')) {
                                        texto.removeClass("auto-no-seleccionado");
                                        texto.addClass("auto-seleccionado");
                                    }
                                    ...
                                    divAux.append("<a style='display:inline;' id=" + val.Codigo + texto.attr('id') + " href=#>" + val.DescripcionCompleta + "</a>" + "<br/>");
                                    bindBehaviors(val)
                                }
                            });
                        };
