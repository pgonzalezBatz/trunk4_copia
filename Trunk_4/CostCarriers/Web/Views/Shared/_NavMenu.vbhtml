    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                @Html.Raw(Utils.GenerarMenu(rolesUsuario))
            </ul>
            <ul class="nav navbar-nav navbar-right">
                <li><a href="@Url.Action("Salir", "Login")"><span class='glyphicon glyphicon-off'></span> @Utils.Traducir("Salir")</a></li>
            </ul>            
            <p class="navbar-text navbar-right"><span class='glyphicon glyphicon glyphicon-user'></span>&nbsp;@Html.Raw(String.Format("<b>{0} ({1})</b>", Model.NombreCompleto, Model.Plantas.FirstOrDefault(Function(f) f.Id = Model.IdPlanta).Nombre))</p>
        </div>
    </div>
</nav>