...
#Region "METODOS"
    ...
    Protected Sub BindDataView()
        Try
            ...
            Dim strSql As String = "select * from cubos.DESPIECES where t2tenr='" & hfReferencia.Value & "' "
            ...
            While dr.Read
                ...
                txtDescref.Text = If(dr("T2BEZ1") Is Nothing OrElse dr("T2BEZ1") Is DBNull.Value, desc_ref, dr("T2BEZ1"))
            ...
            listaType2 = oDocBLL.CargarListaAsociaciones2Ref(hfReferencia.Value, hfComponente.Value) 'work, step
            ...
            For i = 0 To listaType2.Count - 1
                ...
                If listaType4.Count > 0 Then
                    ...
                    listaType2(i).Desc_Process_Planta = listaType4(0).Descripcion
                    listaType2(i).Desc_Process_Cliente = listaType4(0).textolibre
                    listaType2(i).Desc_Process_User = listaType4(0).textolibre2
                Else
                    listaType2(i).desc_process = ""
                    listaType2(i).Desc_Process_Planta = ""
                    listaType2(i).Desc_Process_Cliente = ""
                    listaType2(i).Desc_Process_User = ""
                End If
                ...
                listaType2(i).Desc_step_Larga = lista(0).Descripcion
                If listaTypeStep.Count > 0 Then
                    'mirar desc_comp
                    'mirar la desc de este hijo EN AS400
                    Dim strSql3 As String = "select T2DENO from cubos.DESPIECES where T2KOMP='" & listaType2(i).componente & "'"
                    Dim cn3 As New OleDb.OleDbConnection()

                    cn3.ConnectionString = ConfigurationManager.ConnectionStrings("AS400").ConnectionString
                    Dim cm3 As New OleDb.OleDbCommand(strSql3, cn3)
                    ...
                    While dr3.Read
                        ...
                        listaType2(i).desc_comp = If(dr3("T2DENO") Is Nothing OrElse dr3("T2DENO") Is DBNull.Value, listaType2(i).desc_comp, dr3("T2DENO"))
                    End While
                    ...
                    If arrHijos.Length > 0 Then
                        ...
                        For j = 0 To arrHijos.Length - 2
                            ...
                            item2.referencia = listaType2(i).referencia
                            'item.Id = listaType2(i).componente
                            item2.componente = listaType2(i).componente
                            item2.Process = listaType2(i).Work  'el campo work hace referencia al proceso



                            item2.Steps = listaType2(i).Steps
                            'sacar el work del step de la tabla stepswork
                            Dim listaType3 As New List(Of KaplanLib.ELL.Asociacion)
                            listaType3 = oDocBLL.CargarListaWork(item2.Steps)
                            If listaType3.Count > 0 Then
                                item2.Work = listaType3(0).Work
                            Else
                                item2.Work = 0
                            End If
                            ...
                            While dr2.Read
                                'item.desc_comp = dr2("T2DENO")
                                item2.desc_comp = If(dr2("T2DENO") Is Nothing OrElse dr2("T2DENO") Is DBNull.Value, item2.desc_comp, dr2("T2DENO"))
                            End While
                            ...
                            item2.Caracteristica = listaType2(i).Caracteristica & "<br>" & listaType2(i).Caracteristica2
                            'listaTypeDatos.Add(item)




                            listaType2(i).Hijos = listaType2(i).Hijos & arrHijos(j) & " " & item2.desc_comp & "<br>"
                    ...
                    For m = 0 To listaTypeStep.Count - 1
                        ...
                        If listaTypeStep(m).TipoC <> 666 Then
                            If listaTypeStep(m).TipoC = 999 Then 'atributos. 
                                ...
                                atrib = atrib & "Trabajo STD: " & listaTypeStep(m).TrabajoSTD & "  Parámetro Proceso: " & listaTypeStep(m).Parametro & "  Specification: " & listaTypeStep(m).Caracteristica & "  Clase: " & listaTypeStep(m).Caracteristica2 & "  Máximo: " & listaTypeStep(m).Max & "  Mínimo: " & listaTypeStep(m).Min & "<br>"
                            Else
                ...
                listaType2(i).Desc_work_Larga = listaType2(i).Textolibre2
            ...
            ''a listatype2 hay que añadir un item por cada caracteristica, y luego por modo fallo
            Dim contador As Integer = -1
            Dim listaTypeDatos As New List(Of KaplanLib.ELL.Datos)
            For i = 0 To 1000
                Dim itemvacio As New KaplanLib.ELL.Datos
                listaTypeDatos.Add(itemvacio)
            Next
            'listaTypeDatos.Clear()
            For i = 0 To listaType2.Count - 1
                contador = contador + 1
                'Dim item3 As New KaplanLib.ELL.Datos
                'item3.Referencia = listaType2(i).referencia
                'item3.componente = listaType2(i).componente
                'item3.proceso = listaType2(i).desc_process
                'item3.steps = listaType2(i).Textolibre
                'item3.Work = listaType2(i).Textolibre2
                'item3.elementos = listaType2(i).Hijos
                'item3.Descripcion = listaType2(i).Textolibre3
                'item3.Atributo = listaType2(i).tTipoC


                listaTypeDatos(contador).Referencia = listaType2(i).referencia
                listaTypeDatos(contador).Desc_Ref = txtDescref.Text
                listaTypeDatos(contador).Desc_comp = listaType2(i).desc_comp
                listaTypeDatos(contador).Desc_work = listaType2(i).Textolibre2
                listaTypeDatos(contador).Desc_work_Larga = listaType2(i).Desc_work_Larga
                listaTypeDatos(contador).Desc_step_Larga = listaType2(i).Desc_step_Larga

                listaTypeDatos(contador).componente = listaType2(i).componente
                listaTypeDatos(contador).proceso = listaType2(i).desc_process
                listaTypeDatos(contador).Desc_Process_Planta = listaType2(i).Desc_Process_Planta
                listaTypeDatos(contador).Desc_Process_Cliente = listaType2(i).Desc_Process_Cliente
                listaTypeDatos(contador).Desc_Process_User = listaType2(i).Desc_Process_User

                listaTypeDatos(contador).steps = listaType2(i).Textolibre
                listaTypeDatos(contador).Work = listaType2(i).Work 'Textolibre2
                listaTypeDatos(contador).elementos = listaType2(i).Hijos
                listaTypeDatos(contador).Descripcion = listaType2(i).Textolibre3
                listaTypeDatos(contador).Atributo = listaType2(i).tTipoC



                'busco caracteristicas de ese ref,comp,proc,step
                listaTypeStep = oDocBLL.CargarListaAsociaciones4(listaType2(i).cont, hfReferencia.Value, listaType2(i).componente, listaType2(i).Work, listaType2(i).Steps)
                For k = 0 To listaTypeStep.Count - 1
                    listaTypeStep(k).Max = listaTypeStep(k).Max / 1000
                    listaTypeStep(k).Min = listaTypeStep(k).Min / 1000
                ...
                If listaTypeStep.Count > 0 Then
                    ...
                    For m = 0 To listaTypeStep.Count - 1
                        If m > 0 Then
                            ...
                            listaTypeDatos(contador).Desc_Process_User = listaType2(i).Desc_Process_User
                            listaTypeDatos(contador).steps = listaType2(i).Textolibre
                            listaTypeDatos(contador).Work = listaType2(i).Work
                            listaTypeDatos(contador).elementos = listaType2(i).Hijos
                            listaTypeDatos(contador).Descripcion = listaType2(i).Textolibre3
                            listaTypeDatos(contador).Atributo = listaType2(i).tTipoC
                        End If
                        ...
                        If listaTypeStep(m).TipoC <> 666 Then
                            If listaTypeStep(m).TipoC = 999 Then 'atributos. 
                                'los atributos na hacen linea
                                contador = contador - 1
                                'atrib = atrib & "&nbsp;<tr><td><span style='font-size:small; color: #3366ff;'>" & listaTypeStep(m).TrabajoSTD & "&nbsp;&nbsp;&nbsp;</td><td><span style='font-size:small; color: #3366ff;'>" & listaTypeStep(m).Parametro & "&nbsp;&nbsp;&nbsp;</td><td><span style='font-size:small; color: #3366ff;'>" & listaTypeStep(m).Caracteristica & "&nbsp;&nbsp;&nbsp;</td><td><span style='font-size:small;color: #3366ff;'>" & listaTypeStep(m).Caracteristica2 & "&nbsp;&nbsp;&nbsp;</td><td><span style='font-size:small;color: #3366ff;'>" & listaTypeStep(m).Max & "&nbsp;&nbsp;&nbsp;</td><td><span style='font-size:small;color: #3366ff;'>" & listaTypeStep(m).Min & "</td></tr>"

                            Else

                                caracteristic = "Clase: " & listaTypeStep(m).tClaseC & "  Tipo: " & listaTypeStep(m).tTipoC & "  Característica: " & listaTypeStep(m).Caracteristica & "  Specificacion: " & listaTypeStep(m).Caracteristica2 & "  Máximo: " & listaTypeStep(m).Max & "  Mínimo: " & listaTypeStep(m).Min
                                'item3.caracteristica = caracteristic
                                'item3.codcaracteristica = m
                                ''meter otra linea por cada MF
                                'item3.codModoFallo = 1
                                'item3.ModoFallo = listaTypeStep(m).Condicion1

                                listaTypeDatos(contador).caracteristica = caracteristic
                                listaTypeDatos(contador).codcaracteristica = m
                                'meter otra linea por cada MF
                                listaTypeDatos(contador).codModoFallo = 1
                                listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion1
                                'listaTypeDatos.Add(item3)
                                'aqui debo añadir causas n y efectos 3 y Control_Deteccion
                                'el id de asociacion = listaType2(i).id, el listaTypeDatos(contador).codModoFallo
                                'con esto miro causas, order by id desc 
                                Dim listaTypeControl As List(Of KaplanLib.ELL.Asociacion)
                                Dim listaTypeCausa As List(Of KaplanLib.ELL.Asociacion)
                                listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                If listaTypeCausa.Count > 0 Then
                                    For j = 0 To listaTypeCausa.Count - 1
                                        listaTypeDatos(contador).Causas = listaTypeDatos(contador).Causas & listaTypeCausa(j).Condicion1 & " "
                                    Next
                                    'Control_Deteccion con listaTypeCausa(j).id = causa y listaTypeStep(m).Id = idasociacion  voy a tabla deteccion y saco los n que tienen esa causa
                                    'creo que vale con obsoleto=0  and idCondicion=" & cond & " and idAsociacion = " & id
                                    listaTypeControl = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    For j = 0 To listaTypeControl.Count - 1
                                        listaTypeDatos(contador).Control_Det = listaTypeDatos(contador).Control_Det & " Control Detección: " & listaTypeControl(j).Condicion1 & " Detectabilidad: " & listaTypeControl(j).Work
                                        'listaTypeDatos(contador).Control_Det_Valor = listaTypeDatos(contador).Causas & listaTypeControl(j).Condicion1 & " "
                                    Next
                                    listaTypeDatos(contador).Efectos = listaTypeCausa(0).Condicion3 & " " & listaTypeCausa(0).Condicion4 & " " & listaTypeCausa(0).Condicion5
                                    listaTypeDatos(contador).Control_Pre = listaTypeCausa(0).Condicion2
                                    listaTypeDatos(contador).Control_Pre_Valor = listaTypeCausa(0).Work
                                    listaTypeDatos(contador).Calificacion = listaTypeCausa(0).Textolibre2
                                    'calcular el mayor efecto
                                    If listaTypeCausa(0).TipoC > listaTypeCausa(0).ClaseC Then
                                        If listaTypeCausa(0).TipoC > listaTypeCausa(0).cont Then
                                            listaTypeDatos(contador).Severity = listaTypeCausa(0).TipoC
                                        Else
                                            listaTypeDatos(contador).Severity = listaTypeCausa(0).cont
                                        End If
                                    Else
                                        If listaTypeCausa(0).ClaseC > listaTypeCausa(0).cont Then
                                            listaTypeDatos(contador).Severity = listaTypeCausa(0).ClaseC
                                        Else
                                            listaTypeDatos(contador).Severity = listaTypeCausa(0).cont
                                        End If
                                    End If
                                ...
                                If listaTypeStep(m).Condicion2 <> "" Then
                                    ...
                                    listaTypeDatos(contador).caracteristica = caracteristic
                                    listaTypeDatos(contador).codcaracteristica = m
                                    'item3.codModoFallo = 2
                                    'item3.ModoFallo = listaTypeStep(m).Condicion2
                                    listaTypeDatos(contador).codModoFallo = 2
                                    listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion2
                                    'listaTypeDatos.Add(item3)
                                    listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    If listaTypeCausa.Count > 0 Then
                                        For j = 0 To listaTypeCausa.Count - 1
                                ...
                                If listaTypeStep(m).Condicion3 <> "" Then
                                    ...
                                    listaTypeDatos(contador).codModoFallo = 3
                                    listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion3
                                    listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    If listaTypeCausa.Count > 0 Then
                                        For j = 0 To listaTypeCausa.Count - 1
                                            listaTypeDatos(contador).Causas = listaTypeDatos(contador).Causas & listaTypeCausa(j).Condicion1 & " "
                                ...
                                If listaTypeStep(m).Condicion4 <> "" Then
                                    ...
                                    listaTypeDatos(contador).codModoFallo = 4
                                    listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion4
                                    listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    If listaTypeCausa.Count > 0 Then
                                        For j = 0 To listaTypeCausa.Count - 1
                                            listaTypeDatos(contador).Causas = listaTypeDatos(contador).Causas & listaTypeCausa(j).Condicion1 & " "
                                ...
                                If listaTypeStep(m).Condicion5 <> "" Then
                                    ...
                                    listaTypeDatos(contador).caracteristica = caracteristic
                                    listaTypeDatos(contador).codcaracteristica = m
                                    listaTypeDatos(contador).codModoFallo = 5
                                    listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion5
                                    listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    If listaTypeCausa.Count > 0 Then
                                        For j = 0 To listaTypeCausa.Count - 1
                                            listaTypeDatos(contador).Causas = listaTypeDatos(contador).Causas & listaTypeCausa(j).Condicion1 & " "
                                ...
                                If listaTypeStep(m).Condicion6 <> "" Then
                                    ...
                                    listaTypeDatos(contador).caracteristica = caracteristic
                                    listaTypeDatos(contador).codcaracteristica = m
                                    listaTypeDatos(contador).codModoFallo = 6
                                    listaTypeDatos(contador).ModoFallo = listaTypeStep(m).Condicion6
                                    listaTypeCausa = oDocBLL.CargarListaAsociaciones6(listaTypeStep(m).Id, listaTypeDatos(contador).codModoFallo)
                                    If listaTypeCausa.Count > 0 Then
                                        For j = 0 To listaTypeCausa.Count - 1
                                            listaTypeDatos(contador).Causas = listaTypeDatos(contador).Causas & listaTypeCausa(j).Condicion1 & " "
                                    ...
                                    End If
                                End If
                                'busco modo de fallo de ese ref,comp,proc,step, caracteristica
            ...
            'escribir los registros segun listaTypeDatos
            'primero borro de la tabla datos todo lo de esta referencia y luego escribo
            oDocumentosBLL.GuardarOpeBorrarDatos(hfReferencia.Value)
            If contador = -1 Then
                contador = 0
            End If

            For i = 0 To contador  'listaTypeDatos.Count - 1

                Dim tipo2 As New KaplanLib.ELL.Datos With {
                        .proceso = listaTypeDatos(i).proceso,
                                 .Desc_Ref = listaTypeDatos(i).Desc_Ref,
                .Desc_comp = listaTypeDatos(i).Desc_comp,
                .Desc_work = listaTypeDatos(i).Desc_work,
                                .Desc_step_Larga = listaTypeDatos(i).Desc_step_Larga,
                .Desc_work_Larga = listaTypeDatos(i).Desc_work_Larga,
                 .Severity = listaTypeDatos(i).Severity,
                .steps = listaTypeDatos(i).steps,
                        .Referencia = hfReferencia.Value,
                        .componente = listaTypeDatos(i).componente,
                        .codcaracteristica = listaTypeDatos(i).codcaracteristica,
                        .codModoFallo = listaTypeDatos(i).codModoFallo,
                        .Work = listaTypeDatos(i).Work,
                        .elementos = listaTypeDatos(i).elementos,
                        .caracteristica = listaTypeDatos(i).caracteristica,
                         .ModoFallo = listaTypeDatos(i).ModoFallo,
                          .Atributo = listaTypeDatos(i).Atributo,
                .Descripcion = listaTypeDatos(i).Descripcion
                        }


                'tipo2.Condicion1 = listaTypeCond(i).Textolibre2

                oDocumentosBLL.GuardarOpeDatos(tipo2)
            Next
            listaTypeDatos.RemoveRange(contador, listaTypeDatos.Count - contador)
            'Dim listaTypeDatos2 As New List(Of KaplanLib.ELL.Datos)
            'For i = contador To listaTypeDatos.Count - 1
            '    listaTypeDatos2.Remove(listaTypeDatos(i))
            'Next

            If (listaTypeDatos.Count > 0) Then

                gvType.DataSource = listaTypeDatos
                gvType.DataBind()
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            hfComponente.Value = Request.QueryString("idComp") ' Request.QueryString("idComp")
            'ComprobarAcceso()
    ...
    Protected Sub gvType_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvType.RowCommand
        ...
        If e.CommandName = "Operacion" Then
            ...
            'Response.Redirect("~/OperacionesRef.aspx?idRef=" & hfReferencia.Value & "&idComp=" & IdComp, False)
        ...
        If e.CommandName = "Operacion2" Then
            ...
            Dim IdComp As Int32 = DirectCast(DirectCast(e.CommandSource, System.Web.UI.Control).DataItemContainer.DataKeysContainer, System.Web.UI.WebControls.BaseDataBoundControl).DataSource(CInt(DirectCast(DirectCast(e.CommandSource, System.Web.UI.Control).NamingContainer, System.Web.UI.WebControls.GridViewRow).DataItemIndex)).id
            Response.Redirect("~/OperacionesModRef.aspx?idRef=" & hfReferencia.Value & "&idComp=" & IdComp, False)
            BindDataView()
        End If
    ...
    Protected Sub btnVolver_click(sender As Object, e As EventArgs) Handles btnVolver.Click
        Try
            ...
            Response.Redirect("~/OperacionesRef.aspx?idRef=" & hfReferencia.Value & "&idComp=" & hfComponente.Value, False)
