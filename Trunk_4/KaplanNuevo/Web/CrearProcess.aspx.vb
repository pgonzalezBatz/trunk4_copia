    ...
    Protected Sub BindDataView()
        Try
            ...
            If hfEmpresa.Value <> "" Then
                listaType = oDocBLL.CargarListaEmpresastextoProcess(PageBase.plantaAdmin, hfEmpresa.Value)
            Else
                listaType = oDocBLL.CargarListaEmpresasProcess(PageBase.plantaAdmin)
            End If
...
#End Region
    ...
    Private Sub CargarDocumentos()
        ...
        lista = oDocBLL.CargarListaEmpresasProcess(PageBase.plantaAdmin)
    End Sub
    ...
    Protected Sub gvType_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvType.RowCommand
        Try
            ...
            If e.CommandName = "Desactivar" Then
                ...
                If (oDocumentosBLL.ModificarEmpProcess(TipoTra)) Then
    ...
    Private Sub ConfiguracionProduct(ByVal idDocumento As Integer, Optional ByVal idType As Integer = 0, Optional ByVal idTransmissionMode As Integer = 0)
        Try
            ...
            If idDocumento = 0 Then
                ...
                LimpiarCampos()
            Else
                lista = oDocBLL.CargarTiposEmpresaProcess(idDocumento, PageBase.plantaAdmin)
                ...
                txtNotificar.Text = lista(0).textolibre
                txtFax.Text = lista(0).textolibre2
            ...
            End If
        Catch ex As Exception
            Throw New SabLib.BatzException("Error al mostrar ", ex)
        End Try
    ...
    Protected Sub btnGuardarNuevaReferencia_Click(sender As Object, e As EventArgs) Handles btnGuardarNuevaSolicitud.Click
        Try
            ...
            Dim tipo As New KaplanLib.ELL.Empresas With {.Planta = PageBase.plantaAdmin, .Nif = HdnCIF.Value, .medio = txtNotificar.Text, .Nombre = HdnNombre.Value, .FecSolEnv = dateFechaSol, .FecEnv = dateFechaEnv, .FecRec = dateFechaRec, .preventiva = 0, .telefono = "", .email = "", .fax = txtFax.Text, .subcontrata = DdlSubcontrata.SelectedValue, .contacto = "", .notificar = txtNotificar.Text.ToLower}  '.Autonomo = DdlAutonomo.SelectedValue,
            Dim tipo2 As New KaplanLib.ELL.Kaplan With {
                    ...
                    .Descripcion = CifEmp.Text, ' HdnCIF.Value 'desc
                    .textolibre = txtNotificar.Text,
                    .textolibre2 = txtFax.Text,
                    .Nombre = NomEmp.Text
                    }
            ...
            If flag_Modificar.Value = "0" Then 'creo que no pasa por aqui. esta opción es de modificación
                ...
                If (oDocumentosBLL.GuardarEmpProcess(tipo2)) Then 'sin commit no actualiza emp para insert emd

                    Master.MensajeInfo = ItzultzaileWeb.Itzuli(" ") & " " & HdnNombre.Value & " " & ItzultzaileWeb.Itzuli("se ha guardado correctamente")

                Else
                    Master.MensajeError = ItzultzaileWeb.Itzuli("Un error ha ocurrido cuando se guardaba ") & " " & HdnNombre.Value
                End If
            Else
                If (oDocumentosBLL.ModificarEmpProcess(tipo2, CInt(flag_Modificar.Value))) Then

                    Master.MensajeInfo = ItzultzaileWeb.Itzuli(" ") & " " & HdnNombre.Value & " " & ItzultzaileWeb.Itzuli("se ha modificado correctamente").ToUpper
                    BindDataView()
                Else
                    Master.MensajeError = ItzultzaileWeb.Itzuli("Un error ha ocurrido cuando se modificaba ") & " " & HdnNombre.Value
                End If
            End If
        ...
        Catch ex As Exception
            ...
            Throw New SabLib.BatzException("Error al modificar ", ex)
        End Try
    ...
    Protected Sub btnGuardarNuevaReferencia2_Click(sender As Object, e As EventArgs) Handles GrabarVista2.Click
        Try
            ...
            Dim tipo As New KaplanLib.ELL.Empresas With {.Planta = PageBase.plantaAdmin, .Nif = txtCIF.Text, .medio = TextBox1.Text, .medio2 = TextBox2.Text, .Nombre = txtNombre.Text, .subcontrata = DdlPreventiva2.SelectedValue, .FecEnv = Date.MinValue, .FecRec = Date.MaxValue}
            Dim tipo2 As New KaplanLib.ELL.Kaplan With {
                    .Proceso = 0, 'proceso
                    .Obsoleto = DdlSubcontrata.SelectedValue, 'obsoleto
                    .Descripcion = txtCIF.Text,
                    .textolibre = TextBox1.Text,
                    .textolibre2 = TextBox2.Text,
            ...
            'Master.MensajeError = ItzultzaileWeb.Itzuli("Ya existe con ese Nombre")
            ...
            If (oDocumentosBLL.GuardarEmpProcess(tipo2)) Then 'sin commit no actualiza emp para insert emd
                    ...
                    Master.MensajeInfo = ItzultzaileWeb.Itzuli("Process") & " " & txtNombre.Text & " " & ItzultzaileWeb.Itzuli("Saved")
        ...
        Catch ex As Exception
            ...
            Master.MensajeError = ItzultzaileWeb.Itzuli("Un error ha ocurrido cuando se guardaba ") & " " & txtNombre.Text
        End Try
