...
#Region "Page Load"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        Try
            If Not Page.IsPostBack Then
                cargarListado()
                txtDestino.Attributes.Add("placeholder", itzultzaileWeb.Itzuli("Introduzca el destino a buscar"))
                txtDestino.Focus()
            End If
    ...
    Private Sub BusquedaPersonas_PreRender(ByVal sender As Object, ByVal e As EventArgs) Handles Me.PreRender
        If Not Page.IsPostBack Then
            ...
            itzultzaileWeb.Itzuli(btnActivar) : itzultzaileWeb.Itzuli(labelDDestino) : itzultzaileWeb.Itzuli(labelConfirmDeleteTitleTarifa)
            ...
            itzultzaileWeb.Itzuli(labelLDestino) : itzultzaileWeb.Itzuli(labelLAnno) : itzultzaileWeb.Itzuli(labelLFAvion)
            itzultzaileWeb.Itzuli(rfvTarAvion) : itzultzaileWeb.Itzuli(labelIdaVuelta) : itzultzaileWeb.Itzuli(labelLFHotel)
            itzultzaileWeb.Itzuli(rfvTarHotel) : itzultzaileWeb.Itzuli(labelPorNoche) : itzultzaileWeb.Itzuli(btnGuardarLineaM)
            ...
            itzultzaileWeb.Itzuli(btnEliminarModalLinea) : itzultzaileWeb.Itzuli(labelLFCocheAlq) : itzultzaileWeb.Itzuli(labelPorDia)
        End If
    End Sub
...
#Region "Vista Listado"
    ...
    Private Sub cargarListado()
        Try
            Master.SetTitle = "Tarifas servicios"
            Dim tarifBLL As New BLL.TarifasServBLL
            mvTarifas.ActiveViewIndex = 0
            Dim lTarifas As List(Of ELL.TarifaServicios) = tarifBLL.loadTarifaList(New ELL.TarifaServicios With {.IdPlanta = Master.IdPlantaGestion, .Destino = txtDestino.Text}, Master.IdPlantaGestion)
            If (lTarifas IsNot Nothing) Then lTarifas = lTarifas.OrderBy(Of String)(Function(o) o.Destino).ToList
    ...
    Private Sub gvTarifas_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles gvTarifas.RowDataBound
        ...
        ElseIf (e.Row.RowType = DataControlRowType.DataRow) Then
            Dim oTarif As ELL.TarifaServicios = e.Row.DataItem
            CType(e.Row.FindControl("lblDestino"), Label).Text = oTarif.Destino
...
#Region "Detalle"
    ...
    Private Sub mostrarDetalle(ByVal id As Integer)
        ...
        If (id <> Integer.MinValue) Then
            Dim tarifBLL As New BLL.TarifasServBLL
            Dim oTarif As ELL.TarifaServicios = tarifBLL.loadTarifaInfo(id)
            btnGuardar.CommandArgument = oTarif.Id
            btnEliminar.Visible = True : lnkTarifaLinea.Visible = True
            txtDDestino.Text = oTarif.Destino
    ...
    Private Sub btnActivar_Click(sender As Object, e As EventArgs) Handles btnActivar.Click
        Try
            Dim tarifBLL As New BLL.TarifasServBLL
            Dim oTarifa As ELL.TarifaServicios = tarifBLL.loadTarifaInfo(CInt(btnGuardar.CommandArgument))
            oTarifa.Obsoleta = False
            tarifBLL.SaveTarifa(oTarifa)
            log.Info("Se ha activado de nuevo la tarifa")
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        Try
            If (txtDDestino.Text.Trim = String.Empty Or CInt(ddlNivel.SelectedValue) <= 0) Then
                Master.MensajeAdvertencia = itzultzaileWeb.Itzuli("Introduzca todos los datos")
            Else
                Dim tarifBLL As New BLL.TarifasServBLL
                Dim oTarifa As New ELL.TarifaServicios
                If (btnGuardar.CommandArgument = String.Empty) Then
                    oTarifa.IdPlanta = Master.IdPlantaGestion
                Else
                    oTarifa = tarifBLL.loadTarifaInfo(CInt(btnGuardar.CommandArgument))
                End If
                Dim lTarifas As List(Of ELL.TarifaServicios) = tarifBLL.loadTarifaList(New ELL.TarifaServicios With {.IdPlanta = Master.IdPlantaGestion, .Destino = txtDDestino.Text.Trim}, Master.IdPlantaGestion, False)
                If (lTarifas.Count > 0) Then
                    If ((oTarifa.Id = Integer.MinValue) OrElse (oTarifa.Id <> Integer.MinValue AndAlso lTarifas.Item(0).Id <> oTarifa.Id)) Then
                        Master.MensajeAdvertencia = itzultzaileWeb.Itzuli("Ya existe una tarifa con ese destino")
                        Exit Sub
                    End If
                End If
                oTarifa.Destino = txtDDestino.Text.Trim
                oTarifa.Nivel = CInt(ddlNivel.SelectedValue)
                tarifBLL.SaveTarifa(oTarifa)
                log.Info("Se han guardado los datos de la tarifa")
    ...
    Private Sub btnEliminarModalTarifa_Click(sender As Object, e As EventArgs) Handles btnEliminarModalTarifa.Click
        Try
            ...
            tarifBLL.Delete(idTarifa)
            log.Info("Se ha borrado la tarifa " & idTarifa)
    ...
    Private Sub gvTarifasAnno_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvTarifasAnno.RowDataBound
        If (e.Row.RowType = DataControlRowType.EmptyDataRow Or e.Row.RowType = DataControlRowType.Header) Then
            itzultzaileWeb.TraducirWebControls(e.Row.Controls)
        ElseIf (e.Row.RowType = DataControlRowType.DataRow) Then
            Dim oLinea As ELL.TarifaServicios.Lineas = e.Row.DataItem
    ...
    Private Sub mostrarDetalleLinea(ByVal idTarifa As Integer, ByVal anno As Integer)
        ...
        lblLDestino.Text = oTarifa.Destino
        ...
        If (anno > 0) Then 'Es una modificacion            
            ...
            Dim oLinea As ELL.TarifaServicios.Lineas = oTarifa.LineasTarifa.Find(Function(o As ELL.TarifaServicios.Lineas) o.Anno = anno)
            ddlAnno.SelectedValue = oLinea.Anno
            ddlAnno.Enabled = False
            txtTarifaAvion.Text = oLinea.TarifaAvion
            txtTarifaHotel.Text = oLinea.TarifaHotel
            txtTarifaCocheAlq.Text = oLinea.TarifaCocheAlquiler
        End If
        ShowModalBoxTarifa(True)
    End Sub
    ...
    Private Sub inicializarDetalleLinea()
        txtTarifaAvion.Text = "0" : txtTarifaHotel.Text = "0" : txtTarifaCocheAlq.Text = "0"
    ...
    Private Sub cargarAnnos(bEsModificacion As Boolean)
        ddlAnno.Items.Clear()
        For anno As Integer = 2013 To Now.Year + 1
    ...
    Private Sub btnGuardarLineaM_Click(sender As Object, e As EventArgs) Handles btnGuardarLineaM.Click
        Try
            If (txtTarifaAvion.Text = String.Empty OrElse txtTarifaHotel.Text = String.Empty) Then
                Master.MensajeAdvertencia = itzultzaileWeb.Itzuli("Introduzca todos los datos")
            Else
                Dim tarifBLL As New BLL.TarifasServBLL
                Dim oLinea As New ELL.TarifaServicios.Lineas With {.IdTarifa = CInt(btnGuardarLineaM.CommandArgument)}
                oLinea.TarifaAvion = CDec(txtTarifaAvion.Text)
                oLinea.TarifaHotel = CDec(txtTarifaHotel.Text)
                oLinea.TarifaCocheAlquiler = CDec(txtTarifaCocheAlq.Text)
                oLinea.Anno = CInt(ddlAnno.SelectedValue)
                tarifBLL.SaveTarifaLinea(oLinea, (Not btnEliminarLineaM.Visible))
                log.Info("Se han guardado los datos de la linea de la tarifa " & oLinea.IdTarifa)
                Master.MensajeInfo = itzultzaileWeb.Itzuli("Datos guardados")
    ...
    Private Sub btnEliminarModalLinea_Click(sender As Object, e As EventArgs) Handles btnEliminarModalLinea.Click
        Try
            ...
            tarifBLL.DeleteLinea(idTarifa, anno)
            log.Info("Se ha borrado la linea " & btnGuardarLineaM.CommandArgument & " del año " & anno)
            Master.MensajeInfo = itzultzaileWeb.Itzuli("Elemento borrado")
...
#Region "Replicar"
    ...
    Private Sub btnReplicar_Click(sender As Object, e As EventArgs) Handles btnReplicar.Click
        Try
            ...
            log.Info("Se van a replicar las tarifas en el año " & ddlAnnoRep.SelectedValue)
            solAgenBLL.ReplicarTarifas(CInt(ddlAnnoRep.SelectedValue), Master.IdPlantaGestion)
