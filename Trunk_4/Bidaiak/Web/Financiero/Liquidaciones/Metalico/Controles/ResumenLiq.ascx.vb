...
#Region "Propiedades"
    ...
    Public Event Finalizado(ByVal lHGLiq As List(Of ELL.HojaGastos.Liquidacion), ByVal fEmision As Date)
...
#Region "Carga del control"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        script.AppendLine("try{$('#dtFechaEmi').datetimepicker({showClear:true,locale:'" & ticket.Culture & "',format:'" & Threading.Thread.CurrentThread.CurrentCulture.DateTimeFormat.ShortDatePattern.ToUpper & "'});}catch(err) {}")
    ...
    Private Sub Page_PreRender(ByVal sender As Object, ByVal e As EventArgs) Handles Me.PreRender
        If Not Page.IsPostBack Then
            itzultzaileWeb.Itzuli(labelInfo) : itzultzaileWeb.Itzuli(labelFEmision)
            itzultzaileWeb.Itzuli(btnIntegrar)
        End If
    End Sub
    ...
    ''' </summary>
    ''' <param name="lineas">Lineas a cargar</param>
    Public Sub Iniciar(ByVal lineas As List(Of ELL.HojaGastos.Liquidacion))
        Try
            txtFechaEmision.Text = Now.ToShortDateString
            gvLiquidaciones.DataSource = lineas
            gvLiquidaciones.DataBind()
        Catch ex As Exception
            Dim sms As String = "Error al cargar las hojas seleccionadas en el paso anterior"
...
#Region "Acciones"
    ...
    ''' <summary>
    ''' Se integran los gastos
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Private Sub btnIntegrar_Click(sender As Object, e As EventArgs) Handles btnIntegrar.Click
        Try
            If (txtFechaEmision.Text = String.Empty) Then
                RaiseEvent Advertencia("Debe seleccionar una fecha")
            Else
                ...
                For Each row As GridViewRow In gvLiquidaciones.Rows
                    ...
                    idUser = CInt(CType(row.Cells(5).Controls(0), Label).Text)
                    ...
                    lLineas.Add(linea)
                Next
                RaiseEvent Finalizado(lLineas, CDate(txtFechaEmision.Text))
            End If
        Catch batzEx As BatzException
            RaiseEvent ErrorGenerado(batzEx.Termino)
        Catch ex As Exception
            Dim sms As String = "Error al recoger las hojas de gastos a integrar"
            PageBase.log.Error(sms, ex)
            RaiseEvent ErrorGenerado(itzultzaileWeb.Itzuli(sms))
        End Try
    End Sub
    ...
    '''' <param name="sender"></param>
    '''' <param name="e"></param>    
    'Private Sub btnIntegrar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnIntegrar.Click
    '    Try
