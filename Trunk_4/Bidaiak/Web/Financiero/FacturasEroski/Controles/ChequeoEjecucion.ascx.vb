...
#Region "Iniciar"
    ...
    Public Function Iniciar(ByVal _idUser As Integer) As Boolean
        Try
            ...
            Dim oEjec As BidaiakLib.BLL.BidaiakBLL.Ejecucion = bidaiakBLL.loadEjecucion(BidaiakLib.BLL.BidaiakBLL.TipoEjecucion.Factura_Eroski, idPlanta)
            If (oEjec Is Nothing) Then  'No hay ninguna ejecucion incompleta
                PageBase.log.Info("FACT_AGEN: No existe ningun ejecucion incompleta")
            ...
            Else
                ...
                If (oEjec.IdUser = _idUser) Then  'El usuario tiene iniciada una ejecucion
                    ...
                    PageBase.log.Warn("FACT_AGEN: El usuario " & oEjec.IdUser & " tiene iniciada una ejecucion en el paso " & oEjec.Paso & " en " & fecha.ToShortDateString & "-" & fecha.ToShortTimeString & ".¿Desea continuar?")
                    Return True
                Else 'Existe otro usuario propietario de la ejecucion 
                    ...
                    PageBase.log.Warn("FACT_AGEN: Se ha interrumpido la ejecucion debido a que el usuario " & oEjec.IdUser & " tiene iniciada una ejecucion en el paso " & oEjec.Paso & " en " & fecha.ToShortDateString & "-" & fecha.ToShortTimeString)
                    Return True
                End If
            End If
...
#Region "Botones"
    ...
    Private Sub btnSi_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSi.Click
        PageBase.log.Info("FACT_AGEN: Continua en el paso " & btnSi.CommandArgument)
    ...
    Private Sub btnNo_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnNo.Click
        PageBase.log.Info("FACT_AGEN: No continua asi que se borran los temporales y se empieza de nuevo")
        Try
            Dim bidaiakBLL As New BidaiakLib.BLL.BidaiakBLL
            bidaiakBLL.deleteEjecucion(BidaiakLib.BLL.BidaiakBLL.TipoEjecucion.Factura_Eroski, CInt(Session("IdPlanta")))
            PageBase.log.Info("FACT_AGEN: Informacion de ejecucion borrada")
        Catch batzEx As BidaiakLib.BatzException
            RaiseEvent ErrorGenerado("Al pulsar No, no se ha podido borrar el registro de ejecucion de la planta")
        End Try
    ...
    Private Sub btnCancelar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCancelar.Click
        PageBase.log.Info("FACT_AGEN: Se cancela la ejecucion")
