...
Public Module ExtensionesWebControl
    'Public log As log4net.ILog = log4net.LogManager.GetLogger("root.LocalizationLib")
    Private log As log4net.ILog = log4net.LogManager.GetLogger("root.LocalizationLib")

    <Extension()> _
    Public Sub Ordenar(ByRef WebControl As GridView, Optional ByVal CampoOrden As String = Nothing, Optional ByVal DireccionCampo As SortDirection = SortDirection.Ascending)
        '-----------------------------------------------------------------------------------------------------------------------
        '1ª Solucion.
        'A mejorar:
        ...
        '-----------------------------------------------------------------------------------------------------------------------
        Try
            OrdenarObjetos(WebControl, CampoOrden, DireccionCampo)
    ...
    End Sub
    <Extension()> _
    Public Sub Ordenar(ByRef WebControl As DetailsView, Optional ByVal CampoOrden As String = Nothing, Optional ByVal DireccionCampo As SortDirection = SortDirection.Ascending)
        OrdenarObjetos(WebControl, CampoOrden, DireccionCampo)
    End Sub
    <Extension()> _
    Public Sub Ordenar(ByRef WebControl As CheckBoxList, Optional ByVal CampoOrden As String = Nothing, Optional ByVal DireccionCampo As SortDirection = SortDirection.Ascending)
        OrdenarObjetos(WebControl, CampoOrden, DireccionCampo)
    End Sub

    Private Sub OrdenarObjetos(ByRef WebControl As Object, Optional ByVal CampoOrden As String = Nothing, Optional ByVal DireccionCampo As SortDirection = SortDirection.Ascending)
        If WebControl.DataSource IsNot Nothing AndAlso WebControl.DataSource.Count > 0 Then
            Dim ListaObjetos As New List(Of Object)
            For Each Objeto As Object In WebControl.DataSource
                ListaObjetos.Add(Objeto)
            Next
            WebControl.DataSource = OrdenarLista(ListaObjetos, CampoOrden, DireccionCampo)
        End If
    End Sub
    Function OrdenarLista(ByRef ListaObjetos As List(Of Object), Optional ByVal CampoOrden As String = Nothing, Optional ByVal DireccionCampo As SortDirection = SortDirection.Ascending) As List(Of Object)
        '-----------------------------------------------------------------------------------------------------------------------
        '1ª Solucion.
        'A mejorar:
        '- Poder asignar el "Objeto.DataSource" directamente a una lista o poder tratar el "Objeto.DataSource" como una lista.
        '	Evitando el "FOR EACH" para rellenar la lista.
        '- Obtener directamete el tipo desde el "Objeto.DataSource" o desde la lista sin tener que ir a uno de sus elementos.
        '-----------------------------------------------------------------------------------------------------------------------
        ...
        '    Dim Propiedad As System.Reflection.PropertyInfo = Propiedades.Find(Function(pi As System.Reflection.PropertyInfo) StrComp(pi.Name, CampoOrden.Trim, CompareMethod.Text) = 0)
        '    '-------------------------------------------------------------------------------------------------------------------
        '    'FROGA: Intentar encontrar las propiedades anidadas para poder ordenar por las propiedades de las clases anidadas.
        '    '-------------------------------------------------------------------------------------------------------------------
        '    'Dim Propiedad As System.Reflection.PropertyInfo = Propiedades.Find(Function(pi As System.Reflection.PropertyInfo) StrComp(pi.Name, CampoOrden.Trim, CompareMethod.Text) = 0)
        '    'Dim Propiedad As System.Reflection.PropertyInfo = Nothing
        '    'For Each pi As PropertyInfo In Propiedades
        '    '    'OPERACIONES_TIPO.M_SECCION.SECCION
        '    '    CampoOrden = "M_SECCION"
        '    '    If StrComp(pi.Name, CampoOrden.Trim, CompareMethod.Text) = 0 Then
        '    '        Propiedad = pi
        '    '    End If
        ...
        '            'FROGA: Intentar encontrar las propiedades anidadas para poder ordenar por las propiedades de las clases anidadas.
        '            '-------------------------------------------------------------------------------------------------------------------
        '            ListaObjetos = ListaObjetos.OrderBy(Function(o) If(BuscarPropiedad(o, Propiedad.Name) Is Nothing, Nothing, BuscarPropiedad(o, Propiedad.Name))).ToList
        '            '-------------------------------------------------------------------------------------------------------------------
        ...
        '            'FROGA: Intentar encontrar las propiedades anidadas para poder ordenar por las propiedades de las clases anidadas.
        '            '-------------------------------------------------------------------------------------------------------------------
        '            ListaObjetos = ListaObjetos.OrderByDescending(Function(o) If(BuscarPropiedad(o, Propiedad.Name) Is Nothing, Nothing, BuscarPropiedad(o, Propiedad.Name))).ToList
        '            '-------------------------------------------------------------------------------------------------------------------
        '        End If
        '    End If
        'End If
        '-----------------------------------------------------------------------------------------------------------------------
        'FROGA: Si no da error dejar esto.
        '-----------------------------------------------------------------------------------------------------------------------
        If ListaObjetos IsNot Nothing AndAlso ListaObjetos.Count > 0 Then
            If DireccionCampo = SortDirection.Ascending Then
                ListaObjetos = ListaObjetos.OrderBy(Function(o) If(BuscarPropiedad(o, CampoOrden) Is Nothing, Nothing, BuscarPropiedad(o, CampoOrden))).ToList
            ElseIf DireccionCampo = SortDirection.Descending Then
                ListaObjetos = ListaObjetos.OrderByDescending(Function(o) If(BuscarPropiedad(o, CampoOrden) Is Nothing, Nothing, BuscarPropiedad(o, CampoOrden))).ToList
            End If
        End If
        '-----------------------------------------------------------------------------------------------------------------------
        Return ListaObjetos
    ...
    ''' <summary>
    ''' Funcion para localizar los campos o propiedades por los que se quiere ordenar.
    ''' </summary>
    ''' <param name="ObjetoEntrante">Objeto de la lista que se quiere ordenar</param>
    ''' <param name="Propiedad">Campo o Propiedad del objeto por el que se quiere ordenar.</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function BuscarPropiedad(ByRef ObjetoEntrante As Object, ByRef Propiedad As String) As Object
        Dim ObjetoSaliente As Object
        Dim PosicionPunto As Integer = Propiedad.IndexOf(".")
        Dim Campo As String = If(PosicionPunto > -1, Propiedad.Substring(0, PosicionPunto), Propiedad)
        Dim CampoResto As String = If(PosicionPunto > -1, Propiedad.Substring(PosicionPunto + 1), String.Empty)

        Dim myPropertyID As Reflection.PropertyInfo = ObjetoEntrante.GetType.GetProperties.ToList.Find(Function(pi As Reflection.PropertyInfo) StrComp(pi.Name, Campo, CompareMethod.Text) = 0)
        If myPropertyID Is Nothing Then
            ObjetoSaliente = Nothing
        Else
            ObjetoSaliente = myPropertyID.GetValue(ObjetoEntrante, Nothing)
            If ObjetoSaliente IsNot Nothing And CampoResto <> String.Empty Then ObjetoSaliente = BuscarPropiedad(ObjetoSaliente, CampoResto)
        End If
