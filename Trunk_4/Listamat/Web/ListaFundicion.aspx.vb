Public Partial Class ListaFundicion
    ...
    Private Sub BindMarcas(ByVal ord As Integer, ByVal op As Integer)
        ...
        rptMarcas.DataSource = dbAccess.GetMarcasSinLanzar(1, ord, op, strCn)
        rptMarcas.DataBind()
    End Sub

    Private Sub ListaFundicion_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Dim dbAccess As ListamatLib.IDBAccess = Utilities.GetIDBInstance()
    ...
    Protected Sub Eliminar(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        If Page.IsValid Then
            ...
            Dim dbAccess As ListamatLib.IDBAccess = Utilities.GetIDBInstance()
            dbAccess.EliminarMarca(1, ddlOf.SelectedValue, ddlOp.SelectedValue, imgbtn.CommandArgument, strCn)
        ...
        If Page.IsValid Then
            ...
            For Each i As RepeaterItem In rptMarcas.Items
                ...
                If cb.Checked Then
                    ...
                    dbAccess.LanzarMarca(1, ddlOf.SelectedValue, ddlOp.SelectedValue, ltl.Text, strCn)
                End If
            Next
    ...
    Protected Sub Ordenar(ByVal sender As Object, ByVal e As EventArgs)
        If Page.IsValid Then
            Dim dbAccess As ListamatLib.IDBAccess = Utilities.GetIDBInstance()
            dbAccess.OrdenarMarcas(1, ddlOf.SelectedValue, ddlOp.SelectedValue, strCn)
            BindMarcas(ddlOf.SelectedValue, ddlOp.SelectedValue)
        End If
    End Sub
    Protected Function GetEditLink(ByVal mar As String) As String
        Dim str As New StringBuilder
        str.Append("ListaFundicionMarcas.aspx?ord=")
        If Not Request.Form(ddlOf.UniqueID) Is Nothing Then
