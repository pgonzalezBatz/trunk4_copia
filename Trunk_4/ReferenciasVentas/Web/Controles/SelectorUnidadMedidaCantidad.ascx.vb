...
Partial Public Class SelectorUnidadMedidaCantidad
...
#Region "Propiedades"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property UnidadMedidaCantidad() As String
        Get
    ...
    ''' <summary>
    ''' Identificador de la unidad de medida de cantidad en activo
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property IdUnidadMedidaCantidad() As String
        Get
    ...
    ''' <summary>
    ''' Habilitar/Deshabilitar el textbox de unidad de medida cantidad
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property UnidadMedidaCantidad_Enabled()
        Get
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Event UnidadMedidaCantidadSeleccionado()
...
#Region "Eventos de Pagina"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        aceUnidadMedidaCantidad.OnClientItemSelected = "UnidadMedidaCantidadElegida_" + Me.ID
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "UnidadMedidaCantidadElegida_" & Me.ID, "function UnidadMedidaCantidadElegida_" & Me.ID & "(source, eventArgs){ var hdnValueId = document.getElementById('" & hfUnidadMedidaCantidad.ClientID & "'); hdnValueId.value = eventArgs.get_value();}", True)
...
#Region "Botones"
        ...
        Try
            ...
            If (myButton.CommandName = "A") Then  'Se ha pulsado el boton aceptar
                If (ViewState("IdUnidadSel") IsNot Nothing AndAlso ViewState("DescUnidadSel") IsNot Nothing) Then
                    IdUnidadMedidaCantidad = ViewState("IdUnidadSel")
                    UnidadMedidaCantidad = ViewState("DescUnidadSel")
                    EsValida = True

                    txtUnidadMedidaCantidad.Text = UnidadMedidaCantidad

                    RaiseEvent UnidadMedidaCantidadSeleccionado()
                End If
            Else  ' Se ha lanzado este evento, al perder el foco del cuadro de texto de la referencia
                UnidadMedidaCantidad = txtUnidadMedidaCantidad.Text
                If (txtUnidadMedidaCantidad.Text <> String.Empty) Then
                    Dim oBrainBLL As New BLL.BrainBLL
                    Dim unidadMedida As ELL.BrainBase

                    unidadMedida = oBrainBLL.CargarUnidadMedida(UnidadMedidaCantidad.ToLower, Empresa)

                    If (unidadMedida IsNot Nothing) Then
                        IdUnidadMedidaCantidad = unidadMedida.ELTO
                        UnidadMedidaCantidad = unidadMedida.DENO_S
                        EsValida = True
                        RaiseEvent UnidadMedidaCantidadSeleccionado()
                    Else
                        UnidadMedidaCantidad = String.Empty
                        IdUnidadMedidaCantidad = String.Empty
                        EsValida = False
                    End If
    ...
    ''' <summary>
    ''' Al realizarse el onblur, llama a comprobar la unidad de medida
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
