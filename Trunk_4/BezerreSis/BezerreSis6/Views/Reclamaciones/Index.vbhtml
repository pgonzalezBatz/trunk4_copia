...
@If ViewBag.ShowTable Then
    ...
    @For Each item In Model
        @Code
            ...
            If item.CODIGOGTK IsNot Nothing AndAlso item.CODIGOGTK > 0 Then
                If Not item.FFIN_PREVISTO_E56.Equals(Date.MinValue) AndAlso (item.FFIN_PREVISTO_E56 < item.FECHA_CIERRECLIENTE OrElse (item.FFIN_PREVISTO_E56 < Now.Date And item.FECHA_CIERRECLIENTE = Date.MinValue)) Then
            ...
            If item IsNot Nothing AndAlso item.FECHA_CIERRECLIENTE > Date.MinValue Then
            ...
            Dim afectaIndicadores = If(item.AFECTA_INDICADORES.Equals("1"), "Sí", "No")
        End Code
    ...
    <td><span style=@fechaFinStyle>@(If(item.FECHA_CIERRECLIENTE.Equals(Date.MinValue), "-", item.FECHA_CIERRECLIENTE.ToShortDateString))</span></td>
    <td>@item.CREADOR</td>
    ...
    <script>
        ...
        function atajoTeclado(e) {
            ...
            if (evtobj.ctrlKey && evtobj.shiftKey && evtobj.keyCode == 49) { //ctrl + shift + 1
                ...
                window.location.href = "@Url.Action("IndexTest", "Reclamaciones")"
            }
        }
