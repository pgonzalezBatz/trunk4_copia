...
Partial Public Class MPWeb
	...
	'Public Shared log As log4net.ILog = log4net.LogManager.GetLogger("root.Web")
...
#Region "Propiedades"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Property PerfilUsuario() As ELL.PerfilUsuario
        Get
            If (Session("PerfilUsuario") Is Nothing) Then
                Return Nothing
            Else
                Return CType(Session("PerfilUsuario"), ELL.PerfilUsuario)
            End If
        End Get
        Set(ByVal value As ELL.PerfilUsuario)
...
#Region "Visualizacion de mensajes"
    ...
    ''' <summary>
    ''' Devuelve la cultura de la pagina
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Cultura() As System.Globalization.CultureInfo
        Get
    ...
    ''' <summary>
    ''' Planta en la que se ha iniciado la aplciacion
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public ReadOnly Property Planta() As String()
        Get
            If (Session("Planta") IsNot Nothing) Then
                Return CType(Session("Planta"), String())
            Else
                Return Nothing
            End If
        End Get
...
#Region "Eventos de pagina"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not (Page.IsPostBack) Then
            ...
            'Botón para cerrar
            If (Session("Olanet") IsNot Nothing) Then
        ...
        AddHandler imgSalir.Click, AddressOf imgSalir_Click
        Me.InsertarCssYScripts()
        'Me.Page.Form.DefaultButton = Nothing
    End Sub
    ''' <summary>
    ...
    Protected Sub Page_Render(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        End If
        If (Not Page.IsPostBack) Then
            'itzultzaileWeb.Itzuli(ltlUsuario) : itzultzaileWeb.Itzuli(imgCerrar)
        End If
    End Sub
...
#Region "Métodos"
    ...
    Private Sub InsertarCssYScripts()
        ...
        'jQuery.Attributes("src") = Me.ResolveUrl("~/Scripts/jquery-1.7.1.min.js")
        'Me.cphHead.Controls.Add(jQuery)

        '' Valores núméricos.
        'Dim numericos As New HtmlGenericControl("script")
        'numericos.Attributes("type") = "text/javascript"
        'numericos.Attributes("src") = Me.ResolveUrl("~/Scripts/jquery.numeric.js")
        'Me.cphHead.Controls.Add(numericos)

        '' Global.
        'Dim globalJS As New HtmlGenericControl("script")
        'globalJS.Attributes("type") = "text/javascript"
        'globalJS.Attributes("src") = Me.ResolveUrl("~/Scripts/global.js")
        'Me.cphHead.Controls.Add(globalJS)

        ' Indico la ruta donde está la animación de carga de ThickBox
        'If (Page.ClientScript.IsClientScriptBlockRegistered("rutaLoadingAnimation") <> True) Then
        '    Dim ruta As String = String.Format("var tb_pathToImage = '{0}';", Request.Url.Scheme & "://" + Request.Url.Authority & Request.ApplicationPath & "App_Themes/Tema1/ThickBox/loadingAnimation.gif")
        '    If (File.Exists(ruta)) Then
        '        Dim i As Integer
        '        i = 0
        '    End If
        '    Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "rutaLoadingAnimation", ruta, True)
        'End If
    End Sub
#End Region
   ...
   ''' <summary>
    ...
    Private Sub TratarElementos(ByVal xDocCopia As XmlDocument, ByVal elemento As XmlElement, ByVal elementoCopia As XmlElement)
        ...
        If (PerfilUsuario IsNot Nothing) Then
            rolUsuario = PerfilUsuario.IdTipoTrabajador
            ...
            If (elemento.ChildNodes.Count > 0) Then
                For Each nodo As XmlNode In elemento.ChildNodes
                    ...
                    For Each rol As String In roles
                        rolAux = rol.Trim()
                        If Not (String.IsNullOrEmpty(rolUsuario)) Then
                            If (rolUsuario.Equals(rolAux)) Then
                                existe = True
                            End If
                        End If
                        If (existe) Then
...
#Region "Cerrar session"
    ...
    ' ''' <param name="sender"></param>
    ' ''' <param name="e"></param>    
    'Protected Sub imgCerrar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonTopImage.Click
    '    Try
    ...
    ''' <param name="sender"></param>
    ''' <param name="e"></param>    
    Protected Sub imgSalir_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles imgSalir.Click
        Try
            Session("PerfilUsuario") = Nothing
            'Response.Redirect("~/Login.aspx", False)
            If (Session("Olanet") IsNot Nothing) Then
                Response.Redirect("~/Login.aspx?Olanet=1", False)
                'Session("Olanet") = Nothing
            Else
                Response.Redirect("~/Login.aspx", False)
            End If
        Catch
