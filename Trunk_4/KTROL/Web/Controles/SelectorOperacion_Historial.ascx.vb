...
#Region "Botones"
        ...
        Try
            ...
            Else  ' Se ha lanzado este evento, al perder el foco del cuadro de texto del codigo de operacion
                ...
                If (txtOperacion.Text <> String.Empty) Then
                    Dim linqComp As New KaPlanLib.BLL.LinqComponent
                    Dim regOperacion As Object
                    'Dim regOperacion = linqComp.consultarCodigoOperacion(Referencia, CodigoOperacion)
                    If Not (String.IsNullOrEmpty(Referencia)) Then
                        regOperacion = linqComp.consultarCodigoOperacion(Referencia, CodigoOperacion)
                    Else
                        regOperacion = consultarCodigoOperacion(CodigoOperacion)
                    End If
    ...
    Private Sub consultarOperaciones()
        Try
            If (Session("IdReferencia") IsNot Nothing) Then
                Dim linqComp As New KaPlanLib.BLL.LinqComponent
                '--------------------------------------------------------------------------------------------------------------------------------------------
                gv_Operaciones.DataSource = linqComp.consultarListadoCodigosOperacion(Session("IdReferencia"), GridViewSortExpresion, GridViewSortDirection)
                'FROGA:
                '--------------------------------------------------------------------------------------------------------------------------------------------
                'gv_Operaciones.DataSource = linqComp.consultarListadoCodigosOperacion(Session("IdReferencia"))
                '--------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Public Property GridViewSortExpresion() As String
        Get
            If (ViewState("sortExpresion") Is Nothing) Then
                'ViewState("sortExpresion") = "COD_OPERACION"
