...
Public Class ResumenCalidad
    Inherits PageBase

#Region "Properties"

    ''' <summary>
    ''' La cultura del usuario logueado
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>    
    Private ReadOnly Property CultureUser()
        Get
...
#Region "Page Load"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If Not Page.IsPostBack Then
                If (Session("CodOperacion") IsNot Nothing AndAlso Session("Info") IsNot Nothing AndAlso Session("Registros") IsNot Nothing AndAlso Session("Valores") IsNot Nothing) Then
                    If (VerificarValores()) Then
                        BindDataViews(Session("CodOperacion").ToString(), Session("Info").ToString(), Session("Registros").ToString(), Session("Valores").ToString())
                    Else
...
#Region "Métodos"
    ...
    Protected Sub BindDataViews(ByVal codOperacion As String, ByVal info As String, ByVal registros As String, ByVal valores As String)
        ...
        Try
            ...
            If (IdUsuario <> Integer.MinValue) Then
                ...
                Dim registrosLista As String() = registros.Split(New Char() {","c})
	...
	Private Function cargarCodigosOperacion(ByVal codigoOperacion As String) As List(Of ELL.Caracteristicas_Plan_Fabricacion)
		...
		Catch ex As Exception
			...
			Throw New BatzException("Error al cargar las caracteristicas del plan de fabricación", ex)
		End Try
		Return lista
	End Function
	...
	Private Function cargarCodigosOperacion(ByVal codigoOperacion As String, ByVal registrosLista As String()) As List(Of ELL.Caracteristicas_Plan_Fabricacion)
		...
		Try
			...
			For Each caracteristica In lista
				Dim bool As Boolean = registrosLista.Contains(caracteristica.ID_REGISTRO)
				If (bool) Then
...
#Region "Handlers"
    ...
    Private Function Guardar(Optional ByVal controlError As ELL.ControlesErrores = Nothing) As Integer
        ...
        Try
            ...
            For Each caracteristica In caracteristicas
                'Dim registro As New ELL.ControlesValores
                registro = New ELL.ControlesValores
                Dim posicionValor As Integer = -1
                Dim i As Integer = 0

                If (caracteristica.Responsable_Reg_Cal OrElse caracteristica.Responsable_Reg_Ope) Then
        ...
        Catch ex As Exception
            Global_asax.log.Error("Error al guardar el control por parte de calidad", ex)
            Throw ex
        End Try
    End Function
...
#Region "PRISMA"
	...
	'    Dim oOperacionKaplanPrisma As New BLL.OperacionKaplanPrismaBLL
	'    Try
	'        'Dim codOpe As String = CodigoOperacion
