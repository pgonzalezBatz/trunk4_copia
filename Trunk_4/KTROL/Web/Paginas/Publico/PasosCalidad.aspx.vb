...
Public Class PasosCalidad
    Inherits System.Web.UI.Page

    'Usaremos esta referencia y codigo de operacion provisionales para hacer pruebas
...
#Region "PAGE_INIT"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub PasosCalidad_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ComprobarValores()
    End Sub
    ...
    Private Function TablaAccionesCaracteristica(ByVal i As Integer, ByVal caracteristica As ELL.Caracteristicas_Plan_Fabricacion) As Table
        ...
        If Not (String.IsNullOrEmpty(caracteristica.METODO_CONTROL)) Then
            ...
            ElseIf (caracteristica.METODO_CONTROL.ToLower().Contains("variables") OrElse caracteristica.METODO_CONTROL.ToLower().Contains("var")) Then
                ...
                '-----------------------------------------------------------------------------------
                'Creamos un validador para que solo se puedan introducir Numeros decimales
                '-----------------------------------------------------------------------------------
                '<asp:RegularExpressionValidator ID = "rev_txtVALOR_V" runat="server" 
                'ControlToValidate = "txtVALOR_V" ErrorMessage="advDebeSerNumerico" 
                'ValidationExpression = "^[+-]?(?:\d+\.?\d*|\d+\,?\d*|\d*\,?\d+|\d*\.?\d+)[\r\n]*$" 
                'Display = "None" 
                'ValidationGroup = "vg_dvValor" ></asp:RegularExpressionValidator>

                'Dim rev_txtRegistroDatos As New RegularExpressionValidator
                'rev_txtRegistroDatos.ID = "rev_" & txtRegistroDatos.ID
                'rev_txtRegistroDatos.Display = ValidatorDisplay.Dynamic
                'rev_txtRegistroDatos.ControlToValidate = txtRegistroDatos.ID
                'rev_txtRegistroDatos.ErrorMessage = "advDebeSerNumerico".Itzuli
                'rev_txtRegistroDatos.ValidationExpression = "^[+-]?(?:\d+\.?\d*|\d+\,?\d*|\d*\,?\d+|\d*\.?\d+)[\r\n]*$"
                'rev_txtRegistroDatos.ValidationGroup = "Validacion"

                'celdaMensaje.Controls.Add(rev_txtRegistroDatos)
                '-----------------------------------------------------------------------------------
...
#Region "Consultas"
    ...
    Private Function cargarCodigosOperacion() As List(Of ELL.Caracteristicas_Plan_Fabricacion)
        ...
        Try
            ...
            lista = oConsultasBLL.cargarDatosCodigosOperacionFabricacionPorRol(codigoOperacion, ELL.Usuarios.RolesUsuario.Calidad)
    ...
    Protected Sub btnFinalizar_Click(sender As Object, e As ImageClickEventArgs)
        Try
            ...
            If (registros.Count = wizard_Ktrol.WizardSteps.Count) Then
                ...
                Response.Redirect("ResumenCalidad.aspx", False)
            End If
