...
Public Class PasosOperario
	Inherits System.Web.UI.Page

	Public WithEvents ascx_Mensajes As Mensajes

	Dim idReferencia As Integer = Integer.MinValue
...
#Region "PAGE_INIT"
    ...
    ''' <summary>
    ''' Los valores correspondientes a la característica
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub PasosOperario_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ComprobarValores()
    End Sub
	...
	Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreInit
		Try
			If (Session("CodOperacion") IsNot Nothing AndAlso Session("DescripcionOpe") IsNot Nothing AndAlso Session("Info") IsNot Nothing) Then
				idReferencia = 0
...
#Region "METODOS"
    ...
    Private Sub CreateWizardSteps()
		...
		If Not caracteristicas.Any Then
			'-----------------------------------------------------------------------------------------
			'Al cerrar el mensaje de error volvemos a la pantalla de seleccinar operacion
			'-----------------------------------------------------------------------------------------
			Dim Script As String = "location.href = 'SeleccionRefOp.aspx';"
			Dim mpe_pnlMensaje As AjaxControlToolkit.ModalPopupExtender = ascx_Mensajes.FindControl("mpe_pnlMensaje")
			mpe_pnlMensaje.OnOkScript = Script
			mpe_pnlMensaje.OnCancelScript = Script
			'-----------------------------------------------------------------------------------------
			Dim msg As String = String.Format("No se han encontrado características para realizar el control." & vbCrLf & "Compruebe en Kaplan que existan características para la operación '{0}' y que el  perfil '{1}' las tiene asignadas.",
											  Session("CodOperacion"),
											  [Enum].GetName(GetType(ELL.Usuarios.RolesUsuario), ELL.Usuarios.RolesUsuario.Operario))
    ...
    Private Function CrearDivCabecera(ByVal i As Integer, ByVal caracteristica As ELL.Caracteristicas_Plan_Fabricacion) As System.Web.UI.HtmlControls.HtmlGenericControl
        ...
        If (Not (String.IsNullOrEmpty(rutaImagenCroquis)) AndAlso esImagen(rutaImagenCroquis)) Then
            ...
            If (IdUser() <> Integer.MinValue) Then
                lblOperarioValor.Text = IdUser.ToString()
            Else
    ...
    Private Function CrearDivMain(ByVal i As Integer, ByVal caracteristica As ELL.Caracteristicas_Plan_Fabricacion) As System.Web.UI.HtmlControls.HtmlGenericControl
        ...
        tablaPlano.Style.Add("margin", "auto")
        ...
        celdaDatosCaracteristica.Style.Add("text-align", "center")
    ...
    Private Function TablaDatosCaracteristica(ByVal i As Integer, ByVal caracteristica As ELL.Caracteristicas_Plan_Fabricacion) As Table
        ...
        añadirCelda(filaCabecera, "CLASE", 1, True, True)
    ...
    Private Function TablaAccionesCaracteristica(ByVal i As Integer, ByVal caracteristica As ELL.Caracteristicas_Plan_Fabricacion) As Table
        ...
        tabla.Style.Add("height", "100%")
        tabla.Style.Add("text-align", "center")
        filaMensaje.Style.Add("align", "center")
        filaMensaje.Style.Add("valign", "middle")
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetAltura(ByVal porcAltura) As Integer
        Dim altura As Integer
    ...
    ''' <remarks></remarks>
    Private Function esImagen(ByVal fichero As String) As Boolean
        Dim isImage As Boolean = False
        Dim tiposImagenes As List(Of String) = New List(Of String)(New String() {".jpg", ".png", ".gif", ".jpeg", ".bmp"})
        For Each imagen In tiposImagenes
            If (fichero.ToLower().Contains(imagen)) Then
                isImage = True
            End If
        Next
        Return isImage
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function IdUser() As Integer
...
#Region "Handlers"
    ...
    ''' <summary>
    ''' Se le ha dado al botón Finalizar y se procede a guardar los datos (BOTON NORMAL)
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Protected Sub wizard_Ktrol_FinishButtonClick(sender As Object, e As WizardNavigationEventArgs)
        Dim txt As TextBox = TryCast(wizard_Ktrol.FindControl("txtRegistroDatos0"), TextBox)
        Dim i As Integer = 0
    End Sub
    ...
    Protected Sub btnFinalizar_Click(sender As Object, e As ImageClickEventArgs)
        Try
            ...
            Response.Redirect("ResumenOperario.aspx", False)
        Catch ex As ApplicationException
            ascx_Mensajes.MensajeError(ex)
        Catch ex As Exception
            Global_asax.log.Error(ex)
