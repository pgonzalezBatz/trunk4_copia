...
#Region "Localizacion"
    ...
    Private Shared Function GetUserCulture(ByVal idSab As Integer, ByVal strCn As String) As String
        Dim q = "select idculturas from sab.usuarios where id=:id"
        Dim lstParam As New List(Of Oracle.ManagedDataAccess.Client.OracleParameter)
        lstParam.Add(New Oracle.ManagedDataAccess.Client.OracleParameter("id", Oracle.ManagedDataAccess.Client.OracleDbType.Int32, idSab, ParameterDirection.Input))
        Return OracleManagedDirectAccess.SeleccionarUnico(q, strCn, lstParam.ToArray())
    End Function
#End Region
#Region "Query string functions"
    Public Shared Function AddElementToCommaList(s As String, el As String) As String
        Dim st As Generic.HashSet(Of String) = If(s Is Nothing, New Generic.HashSet(Of String)(), New Generic.HashSet(Of String)(s.Split(",")))
        st.Add(el)
        Return String.Join(",", st)
    End Function
    Public Shared Function RemoveElementToCommaList(s As String, el As String) As String
        Dim st As Generic.HashSet(Of String) = If(s Is Nothing, New Generic.HashSet(Of String)(), New Generic.HashSet(Of String)(s.Split(",")))
        st.Remove(el)
        Return String.Join(",", st)
    End Function
    Public Shared Function ToRouteValues(col As NameValueCollection, obj As Object) As System.Web.Routing.RouteValueDictionary
    ...
    Public Shared Function ToRouteValues(col As System.Web.Routing.RouteValueDictionary, obj As Object) As System.Web.Routing.RouteValueDictionary
        ...
        Return values
    End Function
    Public Shared Function ToRouteValuesDelete(col As NameValueCollection, ByVal ParamArray toRemove() As String) As System.Web.Routing.RouteValueDictionary
        Dim values = New System.Web.Routing.RouteValueDictionary()

        'col.CopyTo(values.ToArray, 0)

        If toRemove IsNot Nothing Then
            For Each k In col.AllKeys
                If Array.IndexOf(toRemove, k) < 0 Then
                    values(k) = col(k)
                End If
            Next
        End If
    ...
    Public Shared Function ModifiQueryString(ByVal oldCollection As Specialized.NameValueCollection, ByVal ParamArray replacements() As KeyValuePair(Of String, String)) As String
        ...
        oldCollection.CopyTo(copy.ToArray, 0)
