...
<asp:Content ContentPlaceHolderID="cphHead" runat="server">
    <title>
        <%=h.Traducir("Pedidos aceptados (lineas)")%> 
    </title>
...
<asp:Content ContentPlaceHolderID="cphContent1" runat="server">
    ...
    <div class="ph1">
        ...
        <form action="enviarcabecera" method="post">
            <fieldset>
            <%=Html.Hidden("pedido", Request("pedido"))%>
            <%=Html.Hidden("fromaction")%>
            <input type="submit" value="<%=h.Traducir("Enviar todas las líneas")%>" />
            </fieldset>
        </form>
        <form action="proponer2cabecera" method="post">
            <fieldset>
            <%=Html.Hidden("pedido", Request("pedido"))%>
            <%=Html.Hidden("fromaction")%>
            <%=h.Traducir("Fecha")%><br />
        ...
        <tbody>
            <%For Each l In Model%>
                <tr>
                    ...
                    <td align="center">
                        <form method="post" action="<%=url.action("enviarlinea") %>">
                            <%=Html.Hidden("pedido", Request("pedido"))%>
