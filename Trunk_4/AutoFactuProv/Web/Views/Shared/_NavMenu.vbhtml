    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                @Html.Raw(Utils.GenerarMenu(Model.IdUser))
            </ul>
            <ul class="nav navbar-nav navbar-right">
                <li><a href="@Url.Action("Salir", "Login")"><span class='glyphicon glyphicon-off'></span> @Utils.Traducir("Salir")</a></li>
            </ul>            
            <p class="navbar-text navbar-right"><span class='glyphicon glyphicon glyphicon-user'></span>&nbsp;@Html.Raw(String.Format("<b>{0}</b>", Model.NombreUsuario))</p>
        </div>
    </div>
</nav>