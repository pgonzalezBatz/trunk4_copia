...
Public Class EstadoTrabajadorDocumento9
    Inherits PageBase
    ...
    Protected Sub BindDataViewlimpia()
        ...
        Try
            ...
            Else
                ...
                gvType.DataSource = Nothing
                gvType.DataBind()
                gvType.Caption = "No hay trabajadores activos"
            End If
    ...
    Protected Sub BindDataView()
        Try
            ...
            Else
                ...
                If Not idTrabajadorParam = 0 Then
                    ...
                    Else
                        If PerfilUsuario.IdRol = ELL.Roles.RolUsuario.Supervisores Then
                            'quito el autorelleno de trabajadores por complex
                            lblTra.Visible = False
                            txtTra.Visible = False
                            trabajadores = oDocBLL.CargarListaTrabajadoresClaveEmpEstadoResponsable(PageBase.plantaAdmin, idTrabajadorParam, intDependientes) 'intDependientes
                        Else
                ...
                Else
                    If PerfilUsuario.IdRol = ELL.Roles.RolUsuario.Supervisores Then
                        trabajadores = oDocBLL.CargarListaTraActivosNoCadResponsable(PageBase.plantaAdmin, intDependientes) 'intDependientes
                    Else

                        trabajadores = oDocBLL.CargarListaTraActivosNoCad(PageBase.plantaAdmin)
                    End If
                ...
                For j = 0 To paginado '  trabajadores.Count - 1 (quito paginacion para mas rapido, solo calcula 10)
                    ...
                    If (listaType2.Count > 0) Then
                        ...
                        For i = 0 To listaType2.Count - 1
                            ...
                            If listaType9.Count > 0 Then
                                listaType2(i).obligatorio = True  'como obligatorio de puesto, miro si es no obligatorio en trabajador luego
                            End If

                            'mirar si en la tabla adok_trd esta trd015 = 0 implica es no obligatorio  listaType2(i).clave
                            '''''''''Dim listaType10 As List(Of ELL.EmpresasDoc)
                            '''''''''listaType10 = oDocBLL.CargarListaTraDocObligatorioTRD(PageBase.plantaAdmin, listaType2(i).clave, trabajadores(j).Id)
                            '''''''''If listaType10.Count > 0 Then
                            '''''''''    If listaType10(0).clave = 1 Then
                            '''''''''        listaType2(i).obligatorio = True
                            '''''''''    Else
                            '''''''''        listaType2(i).obligatorio = False
                            '''''''''    End If
                            '''''''''End If
                                ...
                                Else
                                    ...
                                    Else
                                        ...
                                        If listaType2(i).correcto = 1 Then
                                            no_correcto = 1
                                            listaType2(i).estado = "Marcado Incorrecto"
                                        Else
                    ...
                    Else
                        ...
                        If trabajadores(j).activo > 0 Then
                            ...
                            End If
                        Else
                            trabajadores(j).puesto = "Correcto"
                        End If
                    End If
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Else
                PageBase.empresaBusqueda = 0

                ''''''''' jon tmp lista = oDocBLL.CargarEmpresas()
                ...
                '''''''''        If (listaType2.Count > 0) Then 'son los docs de un puesto
                '''''''''            Dim bucle As Integer
                '''''''''            If listaType2.Count > 19 Then
                '''''''''                bucle = 20
                '''''''''            Else
                '''''''''                bucle = listaType2.Count
                '''''''''            End If
    ...
    Protected Sub gvTypeHis_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvTypeHis.RowCommand
        If e.CommandName = "Desactivar" Then
            ...
            Dim TipoTra As New ELL.Trabajadores With {.Planta = PageBase.plantaAdmin, .Nombre = listaEmpDoc2(0).ubicacion, .activo = 1, .Id = Iddoc2}
            If (oDocumentosBLL.ModificarDOSTra(TipoTra)) Then
                ...
                If listaEmpDoc.Count > 0 Then
                    ...
                    For i = 0 To listaEmpDoc.Count - 1
                        'leo adok_trd_hist con clave listaEmpDoc(i).ubicacion = trd006 y cojo adok_trd004
                        Dim listaType2 As List(Of ELL.TrabajadoresDoc)
                        listaType2 = oDocumentosBLL.CargarListaHistFec(listaEmpDoc(i).ubicacion)
                        If listaType2.Count > 0 Then
                            listaEmpDoc(i).FecRec = listaType2(0).FecIni
                            listaEmpDoc(i).Abrev = listaType2(0).FecIni.ToShortDateString
                        End If
    ...
    Public Sub ibModificar_Click(sender As Object, e As EventArgs)
        Try
            ...
            If txtNombre2.Text.Contains("DNI=") Then
            Else
    ...
    Protected Sub Volver(sender As Object, e As EventArgs) Handles botonVolver.Click
        Try
            If ex177.Text = "Empresa" Then
                ...
                Response.Redirect("~/EstadoEmpresaDocumento9.aspx?hfEmpresa=" & txtEmpresa.Text, False)
            Else
    ...
    Protected Sub btoVolver_click(sender As Object, e As EventArgs) Handles btoVolver.Click
        Try
            If idTrabajadorParam > 0 Then
                Response.Redirect("~/EstadoTrabajadorDocumento9.aspx?empresa=" & idTrabajadorParam & "&pagina=" & gvType.PageIndex, False)
            End If
    ...
    Protected Sub btoSubirDocHist_click(sender As Object, e As EventArgs) Handles SubirDocHist.Click
        Dim oDocBLL As New BLL.DocumentosBLL

        'Dim fila As GridViewRow = CType(CType(sender, Control).Parent.Parent, GridViewRow)
        Dim idDocumento As String = idDocHist ' gvType2.DataKeys(fila.RowIndex)("clave").ToString()
        txtEmpDoc.Text = DescEmpresa.Value
        idDoc.Value = idDocumento
        flagHist.Text = "1"
        flagHistorico.Value = "1"
        ...
        If listaDoc.Count > 0 Then
            ...
            txtDuracion2.Text = listaDoc(0).listacorreos
    ...
    Protected Sub btoVolverEmpresa_click(sender As Object, e As EventArgs) Handles btnVolverEmpresa.Click
        Try
            If Request.QueryString("empresa") <> "" Then
                If idTrabajadorParam > 0 Then
                    Response.Redirect("~/EstadoEmpresaDocumento9.aspx?empresa=" & idTrabajadorParam & "&pagina=" & gvType.PageIndex, False)
                Else
    ...
    Private Sub btnSubir2_Click(sender As Object, e As EventArgs) Handles btnSubir2.Click
        ...
        Try 'ListBox22
            ...
            If FuDoc.HasFile Then
                ...
                If flagHistorico.Value = "1" Then
                    nombreDocum = DirFicherosSubir & "Ficheros_Matriz_Puestos/Documentos/" & prefijo & " " & FuDoc.FileName
                    nombreCorto = prefijo & " " & FuDoc.FileName
                Else
                    nombreDocum = DirFicherosSubir & "Ficheros_Matriz_Puestos/Documentos/" & prefijo & Right(FuDoc.FileName, 5)
                    nombreCorto = prefijo & Right(FuDoc.FileName, 5)
                End If
                FuDoc.SaveAs(nombreDocum)
                ...
                'If flagHistorico.Value = "1" Then
                '    tiposCarne = Trim(txtDuracion2.Text)
                'Else
                tiposCarne = Trim(txtDuracion.Text)
                'End If
                ...
                Dim tipo As New ELL.TrabajadoresDoc With {.tiposCarne = tiposCarne, .Planta = PageBase.plantaAdmin, .periodicidad = periodo, .estado = intestado, .codtra = codtrar, .coddoc = codigo, .FecCad = fechaCad, .FecIni = fechaValidez, .ubicacion = nombreCorto}
                'compruebo si su fecha de caducidad es > que la que existe para ponerlo en historico o no
                'el cod es listaDoc(0).fecini flagHistorico.Value = "1"
                If listaDoc(0).FecIni > fechaValidez Then  'insert adok_trd_hist

                    If (oDocumentosBLL.ModificarTraDoc(tipo, 6, Session("Ticket").nombreusuario)) Then  'actualizamos emd005 y emd004 y emd006
                        Master.MensajeInfo = ItzultzaileWeb.Itzuli("Se ha añadido el documento en el histórico") & " " & codigo & " " & ItzultzaileWeb.Itzuli("al trabajador") & " " & idTrabajador.Value & " " & ItzultzaileWeb.Itzuli("correctamente").ToUpper
    ...
    Public Sub ibSubir_Click(sender As Object, e As EventArgs)
        Try
            ...
            If listaDoc.Count > 0 Then
                txtDocEmp.Text = listaDoc(0).Nombre
                txtDuracion2.Text = listaDoc(0).listacorreos
                txtDuracion.Text = listaDoc(0).listacorreos
                'lo cojo del doc y si hay ya elegido deo de trd lo que haya grabado ddlCaducidad.SelectedValue = listaDoc(0).Periodo
                ddlCaducidad.SelectedValue = listaDoc(0).Periodo
