...
Public Class EstadoTrabajadorDocumentoMedico
    Inherits PageBase
...
#End Region
    ...
    Public Sub ibModificar_Click(sender As Object, e As EventArgs)
        Try
            ...
            If listaDoc2.Count > 0 Then
                ...
                If listaDoc2(0).FecRev > Date.MinValue Then
                    TxtFechaIni.Text = listaDoc2(0).FecRev.ToShortDateString
                Else
    ...
    Protected Sub btoVolverEmpresa_click(sender As Object, e As EventArgs) Handles btnVolverEmpresa.Click
        Try
            If Request.QueryString("empresa") <> "" Then
                If idTrabajadorParam > 0 Then
                    Response.Redirect("~/EstadoTrabajadorDocumentoMedico.aspx?empresa=" & idTrabajadorParam & "&pagina=" & gvType.PageIndex, False)
                Else
    ...
    Protected Sub GrabarMod_Click(sender As Object, e As EventArgs) Handles GrabarMod.Click
        Try
            ...
            If (TxtFechaIni.Text = "") Then
                fechaRevision = Date.MinValue
            Else
                fechaRevision = CDate(TxtFechaIni.Text)
            End If
            ...
            Dim checkcampo As String = listaDoc(0).tiposCarne
            ...
            Dim tipo As New ELL.TrabajadoresDoc With {.FecRev = fechaRevision, .tiposCarne = checkcampo, .Comentario = txtComentario.Text, .Planta = PageBase.plantaAdmin, .FecCad = fechaCad, .FecIni = fechaValidez, .codtra = idTrabajador.Value, .coddoc = idDoc.Value, .FecRec = fechaR, .ubicacion = txtUbicacion.Text, .estado = rblCorrecto.SelectedValue, .Aptitud = 0, .periodicidad = periodo} ', .periodicidad = ddlCaducidad.SelectedValue
