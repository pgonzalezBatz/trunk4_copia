...
Public Class EstadoEmpresaDocumento
    Inherits PageBase
...
#End Region
    ...
    Protected Sub gvType_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvType.RowCommand
        ...
        If e.CommandName = "Trabajadores" Then
            ...
            If PageBase.intRol = 0 Then
                Response.Redirect("EstadoTrabajadorDocumento.aspx?empresa=" & IdEmpre.ToString, True)
            Else
                Response.Redirect("EstadoTrabajadorDocumentoResp.aspx?empresa=" & IdEmpre.ToString, True)
            End If
    ...
    Protected Sub btoVolver_click(sender As Object, e As EventArgs) Handles btoVolver.Click
        Try
            Response.Redirect("~/EstadoEmpresaDocumento.aspx?pagina=" & gvType.PageIndex, False)
    ...
    Protected Sub trabajadores_Click(sender As Object, e As EventArgs) Handles btnTrabajadores.Click
        Try
            ...
            If PageBase.intRol = 0 Then
                Response.Redirect("EstadoTrabajadorDocumento.aspx?empresa=" & idEmpresa.ToString, True)
            Else
                Response.Redirect("EstadoTrabajadorDocumentoResp.aspx?empresa=" & idEmpresa.ToString, True)
            End If
        Catch ex As Exception
            Master.MensajeError = ItzultzaileWeb.Itzuli("Error al ver los trabajadores")
        End Try
