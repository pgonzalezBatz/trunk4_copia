...
Imports IstrikuWebRaiz.PageBase
...
#Region "Property"
	...
	''' Emal del usuario
	''' </summary>	
	Private _email As Boolean = True
	''' <summary>
	...
	''' <value></value>
	''' <returns></returns>	
	Public Property ListaResponsablesSeleccionables() As ListBox
		Get
			Return lsbFiltroUsuarios
		End Get
		Set(value As ListBox)
			lsbFiltroUsuarios = value
		End Set
	End Property
...
#Region "Page_Load"
	...
	Private Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
		txtResponsable.ToolTip = ItzultzaileWeb.Itzuli("textoabuscar") & "." & ItzultzaileWeb.Itzuli("Se puede buscar por nombre, apellidos, nombre de usuario, id de sab o numero de trabajador")
	End Sub
	...
	Private Sub BuscarUsuarios()
		Try
			...
			Dim bConIdDirectorioActivo, bConCodPersona, bConEmail As Nullable(Of Boolean)
			Dim myIdEmpresa As Integer = Integer.MinValue
			If (idEmpresa > 0) Then myIdEmpresa = idEmpresa

			bConIdDirectorioActivo = Nothing : bConCodPersona = Nothing : bConEmail = Nothing
			...
			listaUsuarios = oUserFunc.GetUsuariosBusquedaMultipleSAB(txtResponsable.Text.Trim, bConIdDirectorioActivo, bConEmail, bConCodPersona, Vigentes, myIdEmpresa)

			'----------------------------------------------------------------------
			'Eliminamos de la lista de Filtro los seleccionados.
			'----------------------------------------------------------------------
			If lsbUsuariosElegidos.Items.Count > 0 Then
				For Each item As ListItem In lsbUsuariosElegidos.Items
					If listaUsuarios IsNot Nothing AndAlso listaUsuarios.Any Then
						Dim Id As Integer = item.Value
						listaUsuarios.Remove(listaUsuarios.Find(Function(o As SabLib.ELL.Usuario) o.Id = Id))
					End If
				Next
			End If
			'----------------------------------------------------------------------			
		...
		Catch ex As Exception
			'Dim be As New Sablib.BatzException("Error al cargar los registros".Itzuli, ex)
			Dim be As New SabLib.BatzException("Error al cargar los registros", ex)
...
#Region "Inicializar"
	...
	Public Sub Inicializar()
		...
		lsbFiltroUsuarios.Items.Clear()
		lsbUsuariosElegidos.Items.Clear()
	End Sub
