...
#Region "Eventos de Objetos"
	Private Sub btnAceptar_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles btnAceptar.Click
		Try
			...
			'-------------------------------------------------------------------------------------------------------
			'Identificamos si el documento es nuevo y en que tabla lo estamos creando (GERTAKARIAK o ACCIONES).
			'-------------------------------------------------------------------------------------------------------
			If Documento Is Nothing Then
				Documento = New BatzBBDD.DOCUMENTOS
				Documento.FECHACREACION = Now
				If Master.Propiedades_gvGertakariak.IdSeleccionado IsNot Nothing And gvAcciones_Propiedades.IdSeleccionado Is Nothing Then
					Dim GERTAKARIA As BatzBBDD.GERTAKARIAK = _
										   (From gtk As BatzBBDD.GERTAKARIAK In BBDD.GERTAKARIAK Where gtk.ID = Master.Propiedades_gvGertakariak.IdSeleccionado Select gtk).SingleOrDefault
