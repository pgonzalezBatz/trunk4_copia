    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        Try
            If Not Page.IsPostBack Then
                Master.SetTitle = "Procesar Intereses"
                ...
                btnProcesar.OnClientClick = "return confirm('" & itzultzaileWeb.Itzuli("¿Desea realizar el proceso de separacion de documentos de intereses y mandarlos por email? Puede tardar varios minutos") & "');"
        ...
        Catch ex As Exception
            WriteLog("Error al cargar la pagina de documentos de intereses", TipoLog.Err, ex)
...
#Region "Procesar"
    ...
    ''' <summary>
    ''' Inicia el proceso de separacion de documentos
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>    
    ...
    Private Function ComprobarSiProcesando()
        Dim bComprobando As Boolean = Nomina.estaEjecutandoseProcesoIntereses(Master.IdEmpresaEpsilon)
    ...
    Private Sub VerResultadoUltimaEjecucion()
        Try
            Dim lInfo As List(Of String()) = Nomina.ConsultarEjecucionesProcesoIntereses(Master.IdEmpresaEpsilon)
            ...
            If (sInfo IsNot Nothing) Then
                Dim lResul As List(Of Nomina.ProcesoInteresesTemp) = Nomina.ConsultarDocumentosIntereses(CInt(sInfo(0)), Master.IdEmpresaEpsilon)
                Dim resulOk As Integer = lResul.FindAll(Function(o As Nomina.ProcesoInteresesTemp) o.state = 0).Count
                ...
                mensa.Append("<div class='col-sm-5'><span>" & itzultzaileWeb.Itzuli("Ultima ejecucion realizada") & ":&nbsp;&nbsp;</span><strong><span>" & CDate(sInfo(2)).ToShortDateString & " " & CDate(sInfo(2)).ToShortTimeString & "</span></strong></div>")
                ...
                If (resulError > 0) Then
                    ...
                    Dim paso1 As List(Of Nomina.ProcesoInteresesTemp) = lResul.FindAll(Function(o As Nomina.ProcesoInteresesTemp) o.state <> 0 And o.idSab = Integer.MinValue)
                    Dim paso2 As List(Of Nomina.ProcesoInteresesTemp) = lResul.FindAll(Function(o As Nomina.ProcesoInteresesTemp) o.state <> 0 And o.idSab <> Integer.MinValue)
                    If (paso1.Count > 0) Then
                        mensa.Append("<br /><b>" & itzultzaileWeb.Itzuli("Paso 1:Separacion y proteccion de los intereses").ToUpper & "</b><ul>")
                        For Each resul As Nomina.ProcesoInteresesTemp In paso1
                    ...
                    If (paso2.Count > 0) Then
                        ...
                        For Each resul As Nomina.ProcesoInteresesTemp In paso2
                            mensa.Append("<li>" & resul.mensaje.ToString.Replace(vbCrLf, "<br />") & "</li>")
                        Next
                        mensa.Append("</ul>")
                    End If
                    ...
                    Dim rutaPdf As String = sPlantNomina(1) & "/Intereses/" & anno & "/" & Master.IdEmpresaEpsilon & "_UsuariosConProblemas.pdf"  '& Session.SessionID
    ...
    Private Sub Procesar(ByVal bSimular As Boolean)
        ...
        tipo = "Comienza [TIPO] de documentos de intereses de la planta " & Master.Ticket.IdPlanta
        WriteLog(tipo.Replace("[TIPO]", sTest), TipoLog.Info, Nothing)
        If (Not fuDocumento.HasFile) Then
            Master.MensajeAdvertencia = itzultzaileWeb.Itzuli("Seleccione de nuevo el fichero de intereses")
            Exit Sub
        ...
        Try
            Dim negBLL As New Nomina
            Dim sPlantNomina As String() = negBLL.ConsultarPlantaNomina(Master.IdEmpresaEpsilon)
            Dim rutaPdf As String = sPlantNomina(1) & "\Intereses" '& Session.SessionID            
            WriteLog("Ruta donde se generaran los documentos de intereses temporales:" & rutaPdf, TipoLog.Info, Nothing)
            Dim ruta10T As String = rutaPdf & "\DocIntereses.pdf"
            ...
            LanzarProceso(bSimular, rutaPdf, "DocIntereses.pdf", Now.Year - 1, Master.IdEmpresaEpsilon)  'Cogemos el año anterior            
        Catch batzEx As Sablib.BatzException
            Master.MensajeError = batzEx.Termino
        Catch ex As Exception
            tipo = "Ha ocurrido un error al realizar [TIPO] de documentos de intereses"
        ...
        tipo = "[TIPO] de documentos de intereses finalizado"
        tipo = tipo.Replace("[TIPO]", sTest)
        WriteLog(tipo, TipoLog.Info, Nothing)
    End Sub

    ''' <summary>
    ''' Lanza el ejecutable que procesara los documentos de intereses
    ''' </summary>    
    Private Sub LanzarProceso(ByVal bSimular As Boolean, ByVal ruta As String, ByVal fichero As String, ByVal anno As Integer, ByVal idEpsilon As String)
        ...
        p.FileName = ConfigurationManager.AppSettings("rutaExeDoc")
        p.Arguments = If(bSimular, "S", "P") & " """ & ruta & """ """ & fichero & """ " & anno & " " & txtEmail.Text.Trim & " 1 " & idEpsilon
        p.WindowStyle = ProcessWindowStyle.Hidden
        WriteLog("Lanzar Proceso Intereses:" & p.FileName & " " & p.Arguments, TipoLog.Info, Nothing)
        Process.Start(p)
    End Sub
