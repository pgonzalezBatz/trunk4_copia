...
#Region "Page Load"
    ...
    Private Sub Page_PreRender(ByVal sender As Object, ByVal e As EventArgs) Handles Me.PreRender
        If Not Page.IsPostBack Then
            itzultzaileWeb.Itzuli(labelMesAno) : itzultzaileWeb.Itzuli(labelMes) : itzultzaileWeb.Itzuli(labelTitleDoc)
            itzultzaileWeb.Itzuli(labelAno) : itzultzaileWeb.Itzuli(btnBuscar) : itzultzaileWeb.Itzuli(labelDNI)
            itzultzaileWeb.Itzuli(rfvDNI) : itzultzaileWeb.Itzuli(imgImprimir) : itzultzaileWeb.Itzuli(labelTitleNominas)
        End If
    End Sub
    ...
    Private Sub inicializar()
        pnlResultados.Visible = False : pnlSinResultados.Visible = False : pnlOtrosDocs.Visible = False
        ...
        CargarMesesYAños(fechaCargar.Month, fechaCargar.Year)
        'If (Master.Ticket.IdPlanta = 1) Then
        ...
        '    If (IPLocal) Then
        '        Try
        '            Dim host As System.Net.IPHostEntry = System.Net.Dns.GetHostByAddress(Request.ServerVariables.Item("REMOTE_HOST"))
        '            hfHost.Value = host.HostName.Split(".")(0).ToLower
        '        Catch ex As Exception
        '            WriteLog("Error al consultar el host by address", TipoLog.Err, ex)
        '        End Try
        '    End If
        'End If
    End Sub
    ...
    Private Sub CargarMesesYAños(ByVal month As Integer, ByVal year As Integer)
        Try
            ...
            If (Master.Ticket.IdPlanta = 3) Then 'Rioja pagan la paga extra de julio en junio. Habría que mostrar un mes mas
                mesesPintar = If(Now.Date.Year <> year, 12, Now.Date.Month + 1)
            Else
                mesesPintar = If(Now.Date.Year <> year, 12, Now.Date.Month) 'Si no es el año actual, pintara todos los meses del año
            End If
            ''''check mesesPintar < 1 && > 12 ????? > 12 !!!
            If mesesPintar > 12 Then
                mesesPintar = 12
            End If
            ...
            For counter As Integer = ConfigurationManager.AppSettings("startingYear") To año
                ddlAño.Items.Add(New ListItem(counter, counter))
            Next
            ddlAño.SelectedIndex = ddlAño.Items.IndexOf(ddlAño.Items.FindByValue(year))
        Catch ex As Exception
            Throw New SabLib.BatzException("errIKScargandoFechas:" & month & "/" & year, ex)
        End Try
    End Sub
    ...
    Private Sub rptNominas_ItemDataBound(ByVal sender As Object, ByVal e As RepeaterItemEventArgs) Handles rptNominas.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            Dim imgVer As ImageButton = CType(e.Item.FindControl("imgVer"), ImageButton)
            Dim lnkVer As LinkButton = CType(e.Item.FindControl("lnkVer"), LinkButton)
            'Dim imgPrint As ImageButton = CType(e.Item.FindControl("imgPrint"), ImageButton)
            lblGrupo.Text = nomina(3) & " (" & nomina(4) & ")"
            imgVer.CommandArgument = nomina(2) : lnkVer.CommandArgument = nomina(2)
            'imgPrint.CommandArgument = nomina(2)
            itzultzaileWeb.Itzuli(imgVer) ': itzultzaileWeb.Itzuli(imgPrint)
            'Try
            '    'Solo esta disponible para nominas mayores de septiembre del 2011. Esto es asi porque en la forma de encriptar las nominas, no se metian las contraseñas de usuario y de propiertario asi que son indescriptables desde la aplicacion 
            '    If (hfHost.Value <> String.Empty) Then 'Si tiene algun valor significa que es una IP Local
            '        imgPrint.Visible = (ConfigurationManager.AppSettings("maquinaImpresion").ToLower = hfHost.Value) AndAlso
            '                            ((CInt(ddlAño.SelectedValue) > 2011) OrElse (CInt(ddlAño.SelectedValue) = 2011 AndAlso CInt(ddlMes.SelectedValue) >= CInt(ConfigurationManager.AppSettings("mes2011Imprimible"))))
            '    Else
            '        imgPrint.Visible = False
            '    End If
            'Catch ex As Exception
            '    WriteLog("Error al pintar las nominas en la funcion GetHostByAddress", TipoLog.Err, ex)
            '    imgPrint.Visible = False
            'End Try
        End If
    End Sub
...
#Region "Botones"
    ...
    Private Sub btnBuscar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnBuscar.Click
        Try
            Dim lNominas As List(Of String()) = NominasLib.Nomina.ConsultarNominas(Master.Ticket.IdTrabajador, Master.IdEmpresaEpsilon, ddlMes.SelectedValue, ddlAño.SelectedValue)
            rptNominas.DataSource = lNominas
            rptNominas.DataBind()
            pnlResultados.Visible = (lNominas IsNot Nothing AndAlso lNominas.Count > 0)
    ...
    Protected Sub VerNomina(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            Dim idNomina As Integer = CInt(img.CommandArgument)
    ...
    Private Sub DownloadNomina(ByVal idNomina As Integer)
        Dim bNomina As Byte() = NominasLib.Nomina.DownloadNomina(idNomina)
        Response.Clear()
        Response.AppendHeader("Content-Disposition", "attachment; filename=Nomina.pdf")
        Response.ContentType = "application/pdf"
        Response.OutputStream.Write(bNomina, 0, bNomina.Length)
